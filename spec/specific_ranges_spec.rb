  ...
  it 'Ranges can be used to assign bins, softbins and numbers in the same configuration' do
    TestIds.configure do |config|
      config.bins do |options|
        TestIds.next_in_range(options[:bin_range], options) if options[:bin_range].is_a?(Range)
      end

      config.softbins do |options|
        TestIds.next_in_range(options[:softbin_range], options) if options[:softbin_range].is_a?(Range)
      end

      config.numbers do |options|
        TestIds.next_in_range(options[:number_range], options) if options[:number_range].is_a?(Range)
      end
